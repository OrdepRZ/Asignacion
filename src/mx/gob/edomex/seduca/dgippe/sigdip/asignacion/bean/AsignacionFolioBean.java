...
@ManagedBean
@ViewScoped
public class AsignacionFolioBean implements Serializable {
	...
	public String inicializarAsignacion() {
		...
		try {
			if (aspirante != null) {
				...
				if (aspirante.getStatus() != null && aspirante.getStatus().equals("1")) {
				...
				if (aspirante.getStatus() != null && aspirante.getStatus().equals("1")) {
					if (aspirante.getCveAsignatura() != null && !aspirante.getCveAsignatura().trim().equals("")) {
						if ( (idMunicipio != null && !idMunicipio.trim().equals("") && !idMunicipio.trim().equals("0"))
								|| (idVacante != null && !idVacante.trim().equals("")) || (cct != null && !cct.trim().equals("")) ) {
							List<CampoRptDTO> lstFiltro = new ArrayList<CampoRptDTO>();
							...
							if (lstVacante.isEmpty()) {
								FacesMessage msg = new FacesMessage("No se localizarón vacantes disponibles para la asignatura y filtros seleccionados.");
								msg.setSeverity(FacesMessage.SEVERITY_ERROR);
								FacesContext.getCurrentInstance().addMessage(null, msg);
		...
		try {
			if(vacanteSeleccionada != null  
					...
					if (!existeNombramiento) {
						...
						nombreJASPER = "/jasper/nombramientoba.jasper";
					...
					if (!existeNombramiento) {
						...
						if(firmanteDTO != null){
							...
							/*
							 * implementación sello digital
							 * 
							ArchivoFirmado archivoFirmado = null;
							
							if (loginBean.getUsuario().getLstPermiso().contains("funAsignaSelloDigital")) {
								//Deprecated
								...
								if (archivoFirmado == null) {
									...
									//Atributo descripcionNombramientoProperties
									String nombramientoProperties = "fechaInicio="+ vacanteSeleccionada.getFechaInicio() + "\n" 
																	+ "fechaFin=" + vacanteSeleccionada.getFechaTermino() + "\n"
																	+ "puesto="+ vacanteSeleccionada.getFuncion() + "\n"
																	+ "turno=" + vacanteSeleccionada.getTurno() + "\n"
																	+ "claveCCT=" + vacanteSeleccionada.getCmpCCT() + "\n"
																	+ "descripcionCCT=" + vacanteSeleccionada.getNomCCT() + "\n"
																	+ "subsistema="+vacanteSeleccionada.getDescSubsistema() + "\n";
									
									byte [] nomPropBytes = nombramientoProperties.getBytes();
									
									archivoDTO.setPropiedadesNombramiento(nomPropBytes);									
				...
				if (cveStatus.equals("2") || cveStatus.equals("3")) {
					...
					*/
					...
					if(cveStatus.equals("2") && formato.equals("CHECK")) {
						...
						nombreJASPER = "/jasper/checklist_doc_ba.jasper";
