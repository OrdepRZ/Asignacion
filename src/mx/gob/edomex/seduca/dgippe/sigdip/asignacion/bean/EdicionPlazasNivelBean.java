...
@ManagedBean
@ViewScoped
public class EdicionPlazasNivelBean implements Serializable 
{
	...
	public void consultarVacantes()
	{
		try
		{
			...
			if(cvePresupuestal != null && !cvePresupuestal.equals("") && !cvePresupuestal.equals(cvePresuOriginal))
			{	
				...
				listaVacantesReg = vacanteDAO.getVacantes(lstFiltro);	
		...
		try
		{
			...
			{		
				...
				{				
					...
					//SI ENCUENTRA UN CCT SIN SUBSISTEMA
					else if(idSubCCT == null || idSubCCT.equals(""))
					{
						listaVacantes = new ArrayList<VacanteDTO>();
						FacesMessage message = new FacesMessage("El subsistema del CCT ingresado no corresponde a Educación Básica.");
						message.setSeverity(FacesMessage.SEVERITY_ERROR);
						FacesContext.getCurrentInstance().addMessage(null,message);							
					}						
					else
	...
	public void consultaAsignacion()
	{
		...
		try 
		{
			...
			funcionesVacante = catalogoDAO.getCatalogoItems("ASGFUNASP");
			...
			if(vacante.getFolio() != null && vacante.getFolio() != "" && !vacante.getFolio().equals("0"))
			{
				...
				listaAspirante = aspiranteDAO.getAspirantes(lstFiltro);
	...
	public void cargarAsignaturas()
	{
		try
		{
	        ...
	        {		
	        	...
	        	if(idFunAux == 1 && idAsignAux < 82)
	        	{
	        		asignaturasAux.add(asignatura);
	        	}
	        	//PARA ASIGNATURAS DE ATP
	        	else if(idFunAux == 2 && idAsignAux > 200 && idAsignAux < 213)
	        	{
	        		asignaturasAux.add(asignatura);
	        	}
	        	//PARA ASIGNATURAS DE DIRECTOR
	        	else if((idFunAux == 3 && idAsignAux > 104 && idAsignAux < 109) ||
	        			(idFunAux == 3 && idAsignAux > 154 && idAsignAux < 161) ||
	        			(idFunAux == 3 && idAsignAux > 166 && idAsignAux < 169)	||
	        			(idFunAux == 3 && idAsignAux == 162))
	        	{
	        		asignaturasAux.add(asignatura);
	        	}	  	        	
	        	//PARA ASIGNATURAS DE SUPERVISOR
	        	else if((idFunAux == 4 && idAsignAux > 109 && idAsignAux < 134) ||
	        			(idFunAux == 4 && idAsignAux > 162 && idAsignAux < 167) ||
	        			(idFunAux == 4 && idAsignAux == 161))
	        	{
	        		asignaturasAux.add(asignatura);
	        	}	       	
			...
			FacesMessage msg = new FacesMessage((e.getMessage()!=null ? e.getMessage() : "cargarAsignaturas() : Se generó un error interno en el servidor."));
			msg.setSeverity(FacesMessage.SEVERITY_ERROR);
			FacesContext.getCurrentInstance().addMessage(null, msg);			
		}
	}	
		...
		try 
		{
			...
			{					
				...
				if(msjError.equals(""))
				{
					...
					resultado = vacanteDAO.actualizaVacanteNivel(vacante, cvePresuOriginal ,cveUsuario);
					
					if(resultado.startsWith("00"))
					{
						outcome = "/sigdip/asignacion/seguimiento/edicionPlazaNivel.xhtml?faces-redirect=true";
