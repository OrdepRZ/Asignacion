...
@ManagedBean
@ViewScoped
public class ReversaFolioMSBean implements Serializable {
	...
	public ReversaFolioMSBean() {
		...
		try {
			...
			FacesMessage msg = new FacesMessage((e.getMessage()!=null ? e.getMessage() : "ReversaFolioMSBean() : Se generó un error interno en el servidor."));
		...
		try {
			if (aspirante != null && aspirante.getFolioFederal() != null && !aspirante.getFolioFederal().trim().equals("")) {
				if (observacionReversa != null && !observacionReversa.trim().equals("")) {
					...
					FacesMessage msg = new FacesMessage("Observación de reversa es requerido, por favor verifica.");
					msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					FacesContext.getCurrentInstance().addMessage(null, msg);
				}
			} else {
